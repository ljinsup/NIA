...
public class MongoPersistor extends BusModBase implements
	...
	public void start() {
		...
		readPreference = ReadPreference.valueOf(getOptionalStringConfig(
	...
	private List<ServerAddress> makeSeeds(JsonArray seedsProperty)
	...
	public void handle(Message<JsonObject> message) {
		...
		try {
			...
			// underscores
			// I have kept the version with camel case so as not to break
			// compatibility
	...
	private void doSave(Message<JsonObject> message) {
		...
		if (writeConcern == null) {
			writeConcern = WriteConcern.valueOf(getOptionalStringConfig(
					"write_concern", ""));
		}
		if (writeConcern == null) {
	...
	private void doSaveAll(Message<JsonObject> message) {
		dbName = getMandatoryString("db_name", message);
		String collection = getMandatoryString("collection", message);
		JsonArray doc = message.body().getArray("document");  //getMandatoryObject("document", message);

		if (collection == null || doc == null || dbName == null) {
			return;
		}
		db = mongo.getDB(dbName);
		...
		Iterator<Object> iter = doc.iterator();
		
		while(iter.hasNext()){
			DBObject obj = jsonToDBObject((JsonObject)iter.next());
			WriteResult res = coll.save(obj, writeConcern);
			if (res.getError() != null) {
				sendError(message, res.getError());
				break;
	...
	private void doFind(Message<JsonObject> message) {
		...
		DBCursor cursor;
		if (matcher != null) {
			cursor = (keys == null) ? coll.find(jsonToDBObject(matcher)) : coll
					.find(jsonToDBObject(matcher), jsonToDBObject(keys));
		} else {
			cursor = coll.find();
		}
		if (skip != -1) {
	...
	private void sendBatch(Message<JsonObject> message, final DBCursor cursor,
			final int max, final int timeout) {
		int count = 0;
	...
	private void dropCollection(Message<JsonObject> message) {
		...
		try {
			...
			sendError(message,
					"exception thrown when attempting to drop collection: "
	...
	private void doAggregation(Message<JsonObject> message) {
		...
		AggregationOutput aggregationOutput = dbCollection.aggregate(
				firstPipelineOp, pipelines.toArray(new DBObject[] {}));
